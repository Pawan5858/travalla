...
function fill_dataTable() {
  ...
  var table = $('#table_content').DataTable({
    ...
    "serverSide": true,
    "ajax": {
      "url": baseUrl + "guides/notApproverd/?datatable=true",
      "type": "POST",
      "dataFilter": function (reps) {
        guidesData = JSON.parse(reps);
        return reps;
      }
    },
    createdRow: function (row, data, dataIndex) {
      $(row).data('id', data.gude_id);
    },
    "columnDefs": [
      {
        "targets": '_all',
    ...
    "columns": [
      ...
      },
      {
        ...
        render: function (data) {
          console.log(data)
          return $.fn.dataTable.render.text().display(data.gudt_name);
        }
      },
      {
        "data": "gude_has_car",
        render: function (data) {
          return truncate.apply(getValidData_toDisplay(data).toHTMLEntitiy(), [30, true]);
        }
      },
      {
        "data": "gude_created_at",
        render: function (data) {
          // return truncate.apply(getValidData_toDisplay(data).toHTMLEntitiy(), [30, true]);
          // return $.fn.dataTable.render.text().display(getDateTime_from_dateObject(data));
          return $.fn.dataTable.render.text().display(getDate_from_dateObject(data));
        }
      },

      {
        ...
        "render": function (data, type, row) {
          return `<a class="btn btn-small waves-effect waves-light approvebtn editEventTD" onclick="userApproveAction(this)">Approve</a>`;
        }
      },
      {
        "class": "text-center",
...
function invokeResetPasswordBtnAction(ele) {
  ...
  var arr = $.grep(guidesData.data, function (obj) {
    return obj.gude_id == id;
  });
