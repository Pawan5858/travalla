...

      {
        ...
        render: function (data) {
          ...
          return $('<div>').append($('<button />')
            .prop('type', 'button')
            // .prop('class', 'btn ' + fetchbuttoncolor(data) + ' btn_cstm_status')
...
function typeChanged(ele) {
  ...
  // if($(ele).val() == userTypeEnum.ADMN) {
  //   $('#admin-console-opts-div').removeClass('d-none')
  // } else if($(ele).val() == userTypeEnum.OPTR) {
  //   $('#operator-console-opts-div').removeClass('d-none')
  // }
}
